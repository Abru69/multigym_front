    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 bg-[var(--surface)] p-6 rounded-3xl border border-[var(--border)] shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={() => navigate("/admin/usuarios")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all hover:scale-[1.02] active:scale-95" style={{ background: "rgba(0,0,255,0.1)", color: "var(--accent)", border: "1px solid rgba(0,0,255,0.2)" }}>
            <Users size={14} /> Nuevo Cliente
          </button>
          <button onClick={() => navigate("/admin/rutinas")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all hover:scale-[1.02] active:scale-95" style={{ background: "rgba(255,184,0,0.1)", color: "var(--warning)", border: "1px solid rgba(255,184,0,0.2)" }}>
            <Dumbbell size={14} /> Crear Rutina
          </button>
          <button onClick={() => navigate("/admin/inventario")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all hover:scale-[1.02] active:scale-95" style={{ background: "var(--accent)", color: "var(--accent-text)", boxShadow: "0 0 20px rgba(0,0,255,0.3)" }}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="p-6 rounded-3xl" style={{ background: "var(--surface)", border: "1px solid var(--border)", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
          ...
          <button className="w-full mt-6 py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors hover:bg-white/5" style={{ color: "var(--accent)", border: "1px dashed var(--border)" }}>
