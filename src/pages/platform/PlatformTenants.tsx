...
import { Search, Plus, X, Building2, MoreVertical, CheckCircle, PauseCircle, XCircle, Clock } from "lucide-react"
...
export default function PlatformTenants() {
  ...
  const [form, setForm] = useState({ name:"", subdomain:"", email:"", plan:"STARTER", status:"TRIAL" })
  ...
  const openCreate = () => {
    setForm({ name:"", subdomain:"", email:"", plan:"STARTER", status:"TRIAL" })
    setEditingTenant(null); setShowModal(true)
  }

  const openEdit = (t: Tenant) => {
    setForm({ name:t.name, subdomain:t.subdomain, email:t.email, plan:t.plan, status:t.status })
  ...
  const save = () => {
    if (editingTenant) {
      ...
      setTenants(ts => [newT, ...ts]); showToast("Gimnasio creado")
    }
    setShowModal(false)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          style={{ background:"var(--accent)", color:"#fff", boxShadow:"0 0 20px rgba(0,0,255,0.3)" }}
        >
          ...
          >
            ...
            >
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button onClick={() => setShowModal(false)} className="px-4 py-2 rounded-xl text-sm font-semibold" style={{ background:"var(--surface-hover)", color:"var(--text-secondary)" }}>
                  Cancelar
                </button>
                <button onClick={save} className="px-4 py-2 rounded-xl text-sm font-semibold" style={{ background:"var(--accent)", color:"#fff" }}>
