              ...
              {hasActiveTimer && restTimer && (
                <motion.div
                  ...
                  <MotionButton whileTap={{ scale: 0.9 }}
                    variant="outline"
                    onClick={(e: React.MouseEvent) => { e.stopPropagation(); onSkipRest() }}
                    className="text-[10px] font-bold px-3 py-1 rounded-full h-auto border-border text-text-muted bg-surface hover:bg-surface-hover">
                    Saltar
                  </MotionButton>
                </motion.div>
              )}
        ...
        {!todayRoutine?.isRestDay && totalSets > 0 && (
          <motion.div
            ...
            <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
              {/* Reset */}
              <MotionButton whileTap={{ scale: 0.8 }}
                onClick={resetDayProgress}
                variant="ghost"
                className="p-2 rounded-lg flex-shrink-0 text-text-muted w-10 h-10"
              ...
              {/* Action button */}
              <MotionButton whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.93 }}
                onClick={() => { if (progress < 100) setWorkoutStarted(!workoutStarted) }}
                className="flex items-center gap-2 px-5 py-2.5 h-11 rounded-xl text-xs font-black uppercase tracking-wider whitespace-nowrap"
                style={{
