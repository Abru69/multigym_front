  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      <motion.div variants={fadeUp} className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <MetricCard
          label="ARPU"
          value={formatCurrency(arpuLtv.arpu)}
          icon={Users}
          color="var(--accent)"
        />
        <MetricCard
          label="LTV"
          value={formatCurrency(arpuLtv.ltv)}
          icon={TrendingUp}
          color="var(--warning)"
        />
      </motion.div>
      <motion.div variants={fadeUp} className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <MetricCard
          ...
          value={String(mrr.payingTenants)}
          icon={Users}
          color="var(--success)"
        />
        <MetricCard
          label="Trial Tenants"
          value={String(mrr.trialTenants)}
          icon={Users}
          color="var(--info)"
        />
        <MetricCard
          label="Churn Rate"
          value={`${churnRetention.churnRate}%`}
          icon={TrendingDown}
          color="var(--danger)"
        />
        <MetricCard
          label="Retention Rate"
          value={`${churnRetention.retentionRate}%`}
          icon={TrendingUp}
          color="var(--success)"
        />
      </motion.div>
      <motion.div variants={fadeUp} className="grid grid-cols-3 gap-4">
        <MetricCard
          label="Past Due"
          value={String(mrr.pastDueTenants)}
          icon={AlertTriangle}
          color="var(--warning)"
        />
        <MetricCard
          label="Suspended"
          value={String(mrr.suspendedTenants)}
          icon={AlertTriangle}
          color="var(--danger)"
        />
        <MetricCard
          label="Cancelled"
          value={String(mrr.cancelledTenants)}
          icon={TrendingDown}
          color="var(--danger)"
        />
      </motion.div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                {formatCurrency(p.planPrice)}/month
              </p>
            </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <MetricCard
          label="Churned (30d)"
          value={String(data.churnedLast30Days)}
          icon={AlertTriangle}
          color="var(--warning)"
        />
        <MetricCard
          label="Churned (90d)"
          value={String(data.churnedLast90Days)}
          icon={AlertTriangle}
          color="var(--danger)"
        />
      </div>
          ...
          <tbody>
            {data.recentFailed.map((p, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-right">
                  <Badge variant={p.retryCount >= 3 ? 'destructive' : 'warning'}>
                    {p.retryCount}
                  </Badge>
                </td>
                <td className="px-4 py-3" style={{ color: 'var(--text-secondary)' }}>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-colors hover:bg-[var(--surface-hover)]"
